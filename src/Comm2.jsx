...
import { MyNotesView, RosterView, BoardsView, ToDoView, NamePicker, Nav, ThemedClassWrapper } from "./components.jsx";
...
// Nav tabs (passed to shared <Nav> component) — Comm 2 has no leaderboard
const STUDENT_TABS = [
...
// Comm 2-specific responsive grids (class-specific class names)
...
export default function Comm2() {
  ...
  return <ThemedComm2Wrapper data={data} isAdmin={effectiveAdmin} isGuest={isGuest} view={view} setView={setView} displayName={displayName} testStudent={testStudent} setTestStudent={setTestStudent} setStudentView={setStudentView} studentView={studentView} setUserName={setUserName} effectiveUserName={effectiveUserName} effectiveAdmin={effectiveAdmin} activitiesLive={activitiesLive} setData={setData} />;
}

function ThemedComm2Wrapper({ data, isAdmin, isGuest, view, setView, displayName, testStudent, setTestStudent, setStudentView, studentView, setUserName, effectiveUserName, effectiveAdmin, activitiesLive, setData }) {
  const onLogout = () => {
    if (testStudent) { setTestStudent(null); return; }
    try { localStorage.removeItem(STORAGE_KEY + "-user"); } catch(e) {}
    setUserName(null); setView("home"); setStudentView(false);
  };
  return (
    <ThemedClassWrapper
    ...
    >
      ...
      {view === "todo" && !isGuest && <ToDoView data={data} setData={setData} userName={effectiveUserName} isAdmin={effectiveAdmin} storageKey={STORAGE_KEY} saveData={saveData} accent={ACCENT} lastSortObj={lastSortObj} reboundHours={48} />}
