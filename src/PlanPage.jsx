...
const SESSIONS = [
  {
    ...
    groups: [
      ...
      { name: "The room's keys", items: [
        ...
        ["How the key gets there", "The room screen still never writes to the cast bus with the key in the bundle, so a student with /today open on a phone cannot drive the class. A key pressed there goes two ways at once: on a channel only that browser can hear, for a dashboard in another window of it, and to the server with the instructor PIN, which writes the request onto the live row for a dashboard open on any machine. Andrew: \"yeah need a pin check.\" The first press on a machine with no PIN asks for one, once; a PIN that stops matching asks again."],
        ["Next sends the row's slide", "Andrew: \"when i go left right, even though i have images in teh dashboard, it is only showing text.\" Next and back built their own cast for a row, a plain quote of its words, while the row's own button sent the slide the doc draws. Both send the same slide now: the photo, the article card, the sticky note, the deck. The up-next picture in the rail draws the same slide. An empty line is stepped past."],
      ] },
  ...
  },
  {
    ...
    groups: [
      ...
      { name: "The day's schedule, and Enter and Exit, in the Flow", items: [
        ["Oct 5 and Oct 7 changed places", "Andrew: \"can you flip flop my material for oct 7 and oct 5 in comm 118 please.\" The two day plans swapped keys and week 3's Monday and Wednesday items swapped days, in the store, with a named backup first. OJ is Monday, the celebrity day is Wednesday with Headlines and the two chapters."],
        ["On the schedule today", "Andrew: \"why am i not seeing game and headline on oct 5, or the readings on oct 7? shouldn't the dashboard make this really easy for me to see?\" An activity not yet in a section sat under a fold at the foot of the day, and a reading never reached the Flow at all. Every reading and activity dated to the day is a strip at the top of the Flow now: castable from its row, Add for anything not yet in a section, and a mark on anything already in one. The fold below the day holds slides and what is coming up."],
        ["Enter above the day, Exit below it", "Andrew: \"didn't we work on having an entry and exit part of each day on the dashboard?\" We had. The two buttons came off the room monitor on September 15 and writing the boards moved behind the command bar, which left nothing on the day saying what either board says. Each is a row of the Flow now, in the Enter/Exit colour: the headline and the ideas, Edit in place, and each idea casts from its line. Casting the first Enter idea opens the discussion thread, the way the monitor's button did."],
      ] },
      { name: "The quarter on the bar, and the boards set like the day", items: [
        ["Outline and Map on the bar", "Andrew: \"you gotta put links to the quarter outline and map on the front page of the dashboard.\" Both sat one press inside the date menu, under Plan the quarter. Two buttons on the bar now, beside Around the Horn, and each opens the term on that view."],
      ...
      { name: "Less on the screen", items: [
        ...
        ["Three buttons to the foot of the day", "Andrew: \"we don't need templates, history or meets in person. those can be in the menu for when i click on week 1 sep 21.\" They went into the date menu, then: \"just put those at the bottom of the dashboard.\" Templates, History and Meets in person sit in a row at the foot of the day column now. Only + Section stays on the title row."],
        ...
        ["Enter and Exit take a time, and the foot says what is left", "Andrew: \"i have to be able to give a time estimate for entry and exit, and what i want at the bottom is this: how much time would i have left.\" The two boards carry a time chip like any section, and the total moved from the top of the day to its foot, turned round: 15 to 25 min left of 65, with 40 to 50 planned. Over reads as over by so many minutes."],
        ["One chip lights, not every Wikipedia chip", "Andrew: \"how come when i click on a wikipedia link it lights them all up.\" A chip knew it was on screen by its host name, and every Wikipedia link has the same one. It matches on the address now."],
      ...
      { name: "Take down, above the preview", items: [
        ["A button where it says what is on", "Andrew: \"when something is live on screen, i need a button above the preview on the right to take it off live. i guess i could use the black button huh.\" Esc took the screen down and nothing said so. The strip above the preview that names what is up now has Take down at its right end, in the live red. Black is a different move: the wall goes dark and stays dark until the next cast. Take down puts the idle screen back."],
      ] },
      { name: "Each day is an address", items: [
        ["The dashboard's address names the day", "Andrew: \"what would it take for each day to be a url ... on the dashboard.\" Not much. /comm3/dashboard/oct-7 opens October 7, and picking a day from the date menu, the term popup or the chips writes the address, so a refresh stays put and the back button walks back through the days. A bare /comm3/dashboard still means today, so every existing link keeps working, including the Horn's."],
        ["The spelling that survives the quarter", "Andrew: \"the interesting thing would be how we port it to the next quarter but you can figure that out.\" A term's dates all move and its third Wednesday is still its third Wednesday, so /comm3/dashboard/week-3-wed resolves too, and week-3 alone is that week's first class day. The dashboard only ever writes the date form, because that is what you would type; the week form is for a link that has to outlive the term, from a template, a reminder or the Brief. A date from last quarter names no day of this term, so it opens today's dashboard rather than a dead page."],
      ] },
      { name: "Students", items: [
  ...
  },
  {
    ...
    groups: [
      ...
      { name: "The day plan is a document you type into", items: [
        ...
        ["Slide templates", "The room screen draws every item from a template made for its kind, designed on the Room Slides canvas: a section is a title card; an item is its headline or title with its content; an article is a clipping, or its own picture beside its headline, and Use clipping or Use picture on the row settles which; a video link is its picture with a play button; an image fills the wall; a podcast sits over a waveform; a book chapter is the book beside the chapter's name; a quote is the quote and who said it; an activity is its name, or its parts in columns; Headlines is the word between rules; a game is a ticket with the count of its questions; a question is its options; a board is its prompt and the first sentence of three posts, without names; an assignment is a calendar page with its due date. The same component draws the wall, the slide column and Teach."],
        ["Games, in More", "Run the game is Games, in the dashboard's More and the class site's menu, and The schedule and The room screen lost their articles. The game itself is being rebuilt as decks, one row per answer, so one student's save can never undo another's; the phone, the game panel and the room screen are mocked on the Game Panel canvas."],
        ["Games, one answer per row", "More, Games opens /games: the class's games, Add game, and a setup screen for questions (multiple choice or free-form, an image, the right answer or the answers to accept), individual or teams, time, a closing time and gradebook. Open pushes the game to students: a card comes up over the class site with the game, the box \"I agree not to leave this window during this event\" and Start, and the game also sits in Community. The panel runs it live: All questions, Scores with the average, a question's picked and Please review per answer, Accept with what it changes, free-form answers grouped, Close with a confirm, then Release scores, Release answers and Take it later. Put on screen sends time left and submitted, Spread of scores, All questions (click a question to open it) and teams to the room screen. Every answer is its own database row that only its student can write, so no save can undo another. The code lives in the decks package, installed at v0.2.0; the spring game stays at /rungame for now."],
        ["Games, easier to get around", "Every game sits in a list down the left of every games screen, so getting back to one, or over to another, is one click. The Games table shows each game's questions, when it last ran, how many finished out of the class, the average score and the fall days it sits on in the day plan. Save is the setup's main button and never opens anything; Open asks first. A draft opens straight into its setup. Spring's nine games and their 1,398 answers moved in, closed and out of students' sight, and Games is in the class site's More tab. Decks v0.3.0."],
        ["Run a game, and run it again", "A game is built once and run for each class or section. On a game's page, Run lists every class this term, and a class with two sections as each section; choosing one opens a run of the game to those students only. A run keeps its own copy of the questions and right answers and holds that sitting's answers, approval stream, closing and releases, so the 8:00 and the 10:30 each get a run, and next term gets another, without touching the last. A closed run has Run above Release. An answer approved in one run is accepted in the game, so the next section starts with it. The Games table shows each game's last run and how it went, with the day plan as the second column; a game's page lists all its runs. Slash commands now show a game's name on the line right away."],
        ["Games come from the game panel", "The games built in Games are the only games a day can hold. In the day plan, /game lists them and @ finds them; the drawer's Activities shelf lists them to drag onto a day or Add to a day; the old Game and Team Trivia rows and the game sets in the library are no longer offered. A game row wears the game's name, its kind opens it in Games, its slide is the game's ticket with its question count, and students see it on the schedule. The Games table's Day plan column reads the days whose rows hold the game, with nothing to link. The six fall days that held an old game set get draft games of their own, by a script."],
        ["The approval stream", "Free-form answers that match no right answer come to the game panel in a column beside All questions, in the order they arrived, while students are still taking the game. The same words for the same question arrive once with a count, and one a slip away from a right answer says which. Approve or Deny decides every answer with those words, now and later; the latest decisions sit under Decided with Undo, and a Names switch shows who answered. Answers still waiting stay out of every score until decided, and they keep waiting after Close. Decks v0.4.0."],
        ["No words he did not write, nothing in the corners", "Every word on a slide is read off the item, the block or the class. A field that is empty is left out rather than filled. No kind label, no activity tag, no link parked in the bottom right; a link sits on the words that already name the thing, so an article's site and title, a video's picture, a quote's name and an assignment's title all open what they point at."],
        ["Paper or slate, per class", "Beside Hide slides, Paper and Slate set the ground the room screen and every slide use, for this class. Slate is where every class starts, so nothing on the wall changed on its own."],
        ["Notes on the slide, item by item", "An item's notes stay off its slide until asked: right-click, or / on the line, Show notes on slide. A few short notes sit beside the title; a longer list runs under it."],
        ["Commands without the articles","Put on screen, Open in new tab, Edit, Create slide, Remove slide, Mark done, Make note, Remove from day. Andrew asked for “the” to come out of every command."],
        ["The slash menu", "Type / in any line and a menu opens under it, filtered as you type: make the line a section, an item or a note; give it any kind (article, video, study, game); make it an activity (Headlines, Game, Fishbowl, Around the Horn); put it on screen; create a slide for a note; mark it done; add it to readings; edit details; delete it; open Teach, Templates or History. On a section: add item, move it, delete it. Arrows choose, Enter or Tab takes the choice, Escape closes it. The /word comes back out of the line."],
        ["@ finds what you already have", "The slash menu makes or does something; @ finds something already in the library and puts it here. Type @ and a few words: matching articles, games, items and videos come up by kind. On an empty line the line becomes that thing; on a line with words, it arrives on the line below."],
        ["Shortcuts", "# and a space at the start of a typed line turns it into a section, and the lines after it go with it, the way a heading splits a document. - and a space at the start of an item makes it a note. ⌘Enter marks a line done."],
        ["Sections drag", "A section has a handle beside its name: drag it onto another section, the top half to go above and the bottom half below. Alt+↑ and Alt+↓ in a section's name move it one place."],
        ["Time on sections, in ranges", "Each section has a place for its minutes, as one number or a range: 10, 5-10, 5–10 min. The top of the day adds both ends up, 25–35 min planned of 65, and says how many sections have no time. The total turns red when even the short end runs past the class."],
        ["A time used to vanish", "normSlot, which every write to a section goes through, passed a section's title and note along and dropped everything else. A time would have been erased by the next item added to its section. It passes the whole section through now, and a test holds that."],
        ["Teach", "Teach turns the day into a running order: where you are and the section's time, the title large with the content and the notes under it to read from, the slide beside it, what is next, and the three after. Space or → goes forward and puts the next slide up, ← goes back, Escape leaves. It opens on what is on the screen, or on the next thing not done."],
        ["Templates", "Save any day as a template under a name, and apply one to any day: its sections, names, times and rows arrive after what the day already has, pointing at the same blocks and activities, with nothing on the day replaced. Templates live in the shared store, so COMM 118's are there in COMM 3."],
        ["History", "Every version of a day there is to go back to: the ones this browser kept as the day was edited, at most one a minute, and the store's backups of the whole class from the start of each of the last seven days and before big changes. Each says how many sections, items and notes it holds and its first lines, and Restore writes it over the day, which Undo takes back."],
        ["A link in a line you can press","A text box cannot hold a link that works, so a line with a web address in it shows as text, with the address as a real link, until the cursor goes in. Press the link and a menu asks: put it on the room screen, open it in a new tab, or edit the line. Press anywhere else in the line and the cursor lands where you pressed. The arrows still walk into it."],
        ["Choose an item's kind by pressing it", "The kind beside an item (item, article, video) opens the list of kinds. Choosing one changes the block. A line typed straight onto the day had no kind at all; choosing one makes it a block of that kind in the class, with the line's words as its title and its web address as its link, so it turns up in the repository."],
        ["A slide for a note", "Notes have no slide unless you give them one: right-click a note, Give it a slide. Its slide stacks under its item's, and Remove its slide takes it away."],
        ["An item takes its notes with it","Moving an item moved its row and nothing else, so the notes under it stayed where they were and became notes on whatever item came before. Dragging an item on the day, Move to a day, and dragging across days in the outline now all lift the item with every note under it. An item dropped among another item's notes lands after them, so it cannot split them up. A note dragged on its own still moves alone."],
        ["Sequences are gone", "Andrew was not using them. The Structure picker came off the top of the day and out of Build the day, and every day reads as its own sections. The sequence's empty slots, an opener with nothing in it, no longer draw; five COMM 118 sections that held rows but were only ever named by the sequence (opener, problem) read Section 1, Section 2 like any other nameless section. Nothing in the store was changed, so bringing sequences back is one function. A seed added to a day from the schedule lands in the day's first section."],
        ["Items and notes","Andrew renamed the two lower levels. The kind that was called Note is called Item, in the day, the drawer's shelf, New item and the repository. What were called comments, the lines under an item, are called notes. The day note, My notes and Notes for students keep their names, since they are other things."],
        ["Notes move", "A note under an item is all text box, and a press on a text box selects words, so there was nothing to drag it by. A handle shows beside a note under the pointer; drop in the top half of another note to go above it and the bottom half to go below. Alt+↑ and Alt+↓ move the note you are typing in, and past its item it becomes a note on the next item over."],
        ["Links that go on the room screen", "Paste or type a web address into any line and a link appears beside it once the line saves: press the site's name and the page goes up on the room screen, press ↗ to open it in a tab. Press the name again while it is up to take it down. The links an item already had work the same way; they used to only open a tab."],
      ] },
      { name: "The students' schedule", items: [
        ...
        ["Where each reading came from", "The source written on the reading, or the site its link points at. None of COMM 118's 64 articles has a source written on it, so for now every one shows its site."],
        ["Games and Headlines come from the day plan", "Students see four things on a week: games, Headlines, readings and assignments. The games and Headlines are read off each day's plan, where the class is actually built. The week had its own activity rows, copied in from Spring, and they never followed the day: COMM 118 listed a Game on seven weeks while the games were really Weekly Game sets placed on other days. A set counts as a game when its name or its questions say game or trivia, so a survey set does not. Fishbowl, This or That and the other activities are off the student schedule."],
        ["Deleting a week can be undone", "The ✕ on a week took it and everything on it in one press, and that is how COMM 118 lost week one this afternoon. It was put back from the backup taken before the Spring notes move. Deleting now leaves a bar with Undo, which puts the week back where it was."],
      ] },
      { name: "The day plan reads as a document, with a slide beside each block", items: [
        ["Two columns", "The day plan and the outline are a document on the left, numbered blocks under section headings, and a 200px column of slides on the right. The rule under every row came off, because rows ruled like a spreadsheet read as a table. The kind word moved beside the title instead of sitting in a column at the right edge. Hide slides closes the column, in the day plan and the outline both, and this browser remembers."],
        ["A slide is the cast", "No second slide system. Each block's slide is the cast its arrow sends, drawn by the room screen's own code and scaled to the column, so what sits beside a block is what the room sees. A photo shows the photo, a note or a typed row shows its headline or its words, an activity shows its own screen, a game shows its title card. Three are drawn differently from the wall on purpose: a link shows as its card rather than the live page, a clip shows its first frame without playing, and a voice memo shows its headline. Slides draw only once they scroll near the screen."],
        ["Pressing a slide puts it up", "In the day plan the slide is the button, so the arrow on each row is gone while slides are showing. In the outline slides are a preview. With slides hidden the arrows come back."],
        ["Notes under a block have no slide", "And a block and its notes are one group with the slide beside the group. Laid out a row at a time, a block's slide made its row a slide tall and pushed its notes below the slide, further from the block. The first screenshot showed exactly that."],
        ["Closer, and slightly indented", "A block's content now sits directly under its title, indented a little past the words, and notes under a block sit 30px in and closer together."],
      ] },
  ...
  },
  {
    ...
    groups: [
      { name: "What landed", items: [
        ...
        ["The deck", "A released grade is a full-screen card the next time the student opens the class. Top to bottom: the assignment and its due date, the letter with the date graded and a sentence on what that letter means, Andrew's comment, then what the student turned in under that with their note, their file and the time they sent them. Two ways on: open the assignment, or make a meeting, which posts the same request the You card posts and opens the scheduling link when the class has one. One card per assignment, and the site waits behind the stack. Got it writes a seen stamp, so the card comes round once. Andrew tried the first cut as Theo and asked for a smaller letter, the meaning, the links, the note, the times and the meeting, so the card grew from four lines to this."],
        ["Grades so far", "The You card's percent is gone, in both places. In its place, a parade: every assignment as a tile, light grey with its title until a grade comes in, then the class colour with the letter on top and the title small underneath. The tile in the grid shows the same parade small. A number from the older grading flow reads as the letter its band gives, 90 and up an A, so every tile speaks one language. Andrew asked for this after reading the card as Theo, and answered four questions on the shape."],
