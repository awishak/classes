...
/* ─── ASSIGNMENTS TAB ─── */
function AssignmentsView({ data, setData, isAdmin }) {
      ...
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {assignments.map(a => {
            ...
            return (
              <div key={a.id} style={{ ...crd, padding: 16 }} onClick={() => isAdmin && !isEdit && setEditId(a.id)}>
                {isEdit ? (
...
/* ─── GRADEBOOK ─── */
function Gradebook({ data, setData, userName, isAdmin }) {
  const assignments = data.assignments || DEFAULT_ASSIGNMENTS;
  const grades = data.grades || {};
  const participation = data.participation || {};
  const [selStudent, setSelStudent] = useState(isAdmin ? null : null);
  const [msg, setMsg] = useState("");
  const showMsg = m => { setMsg(m); setTimeout(() => setMsg(""), 2000); };
