...
import { MyNotesView, BioView, RosterView, ReadingsView, BoardsView, ToDoView, NamePicker, Nav, ThemedClassWrapper } from "./components.jsx";
...
function InstructorCard({ data, setData, isAdmin }) {
  ...
  const startEdit = () => {
    ...
    setEditCourseTitle(data.courseTitle || "Comm and Sport");
    setEditing(true);
  };
...
function HomeGradedNotifications({ data, setData, studentId }) {
  ...
  // Find assignments that have been graded and have unseen notifications
  const items = [];
  assignments.forEach(a => {
  ...
  const dismiss = async (key) => {
    ...
    await window.storage.set("comm118-game-v14", JSON.stringify(updated), true);
    setData(updated);
  };
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
        <div style={{ fontSize: 10, fontWeight: 500, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.1em" }}>Recently Graded</div>
        {items.length > 1 && (
      ...
      {items.map(item => {
        ...
        return (
          <div key={item.key} style={{ ...crd, padding: 14, marginBottom: 6, borderLeft: "4px solid " + (isZero ? "#dc2626" : "#10b981"), background: isZero ? "#fef2f2" : "#ecfdf5" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: isZero ? "#991b1b" : "#065f46" }}>
                  {item.assignment.name} has been graded
                </div>
                <div style={{ fontSize: 18, fontWeight: 900, color: isZero ? "#dc2626" : "#111827", marginTop: 4 }}>
                  {item.grade.score}<span style={{ fontSize: 12, color: "#9ca3af", fontWeight: 500 }}> / {item.grade.outOf || 100}</span>
                </div>
                {item.grade.comment && (
                ...
                {isZero && (
                  <div style={{ fontSize: 12, color: "#dc2626", marginTop: 6, fontWeight: 600 }}>This assignment needs attention. Check the requirements and resubmit.</div>
                )}
              </div>
              <button onClick={() => dismiss(item.key)} style={{ fontSize: 20, color: "#9ca3af", background: "none", border: "none", cursor: "pointer", padding: "0 4px", lineHeight: 1 }}>x</button>
            </div>
          </div>
        );
      })}
        ...
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          ...
          <div style={{ ...crd, padding: 16, marginBottom: 16 }}>
            ...
            {[...sessions].reverse().map(s => {
              ...
              const isLive = s.activeHeadlineId && s.phase !== "done";
              return (
                <div key={s.id} style={{ ...crd, padding: 14, marginBottom: 4, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                  <button onClick={() => setActiveSession(s.id)} style={{ flex: 1, background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: TEXT_PRIMARY }}>{s.name}</div>
                      {isLive && <span style={{ fontSize: 9, fontWeight: 800, color: "#065f46", background: "#d1fae5", padding: "2px 6px", borderRadius: 4, textTransform: "uppercase", letterSpacing: "0.08em" }}>Live</span>}
                    </div>
                    <div style={{ fontSize: 11, color: TEXT_MUTED }}>{count} headline{count !== 1 ? "s" : ""} / {new Date(s.ts).toLocaleDateString()}</div>
                  </button>
                  {isLive && <button onClick={(e) => { e.stopPropagation(); closeSession(s.id); }} style={{ ...pillInactive, color: RED, fontSize: 11, padding: "4px 10px" }}>End</button>}
                </div>
              );
            })}
          </div>
    ...
    <div style={{ padding: "20px 20px 40px", fontFamily: themedHeadingFont(theme, F) }}>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <MyNotesView data={data} setData={setData} isAdmin={isAdmin} userName={userName} storageKey={STORAGE_KEY} saveData={saveData} />
        </div>
...
export default function Comm118() {
  ...
  if (!userName) return <NamePicker data={data} onSelect={name => { setUserName(name); setView(name === GUEST_NAME ? "schedule" : "home"); }} storageKey={STORAGE_KEY} allStudents={ALL_STUDENTS} accent={ACCENT} adminPin="118711" lastSort={lastSort} iconText="118" iconFontSize={16} courseCode="COMM 118 · Spring 2026" courseTitle="Communication and Sport" meetingInfo="MWF 8:00 to 9:05 am · Vari 128" />;
...
function ThemedComm118Wrapper({ data, isAdmin, isGuest, view, setView, displayName, testStudent, setTestStudent, setStudentView, studentView, setUserName, effectiveUserName, effectiveAdmin, activitiesLive, visibleStudents, setData }) {
  const onLogout = () => {
    if (testStudent) { setTestStudent(null); return; }
    try { localStorage.removeItem(STORAGE_KEY + "-user"); } catch(e) {}
    setUserName(null);
  };
  return (
    <ThemedClassWrapper
      data={data} setData={setData} view={view} setView={setView}
      isAdmin={isAdmin} effectiveAdmin={effectiveAdmin} isGuest={isGuest}
      displayName={displayName} effectiveUserName={effectiveUserName}
      testStudent={testStudent} setTestStudent={setTestStudent}
      studentView={studentView} setStudentView={setStudentView}
      activitiesLive={activitiesLive}
      storageKey={STORAGE_KEY} accent={ACCENT} onLogout={onLogout}
      defaultTitle="Comm and Sport"
      caminoUrl="https://camino.instructure.com/courses/117721"
      studentTabs={STUDENT_TABS} adminTabs={ADMIN_TABS}
    >
      ...
      {/* Admin extras */}
      {view === "todo" && isAdmin && !studentView && !testStudent && <ToDoView data={data} setData={setData} userName={effectiveUserName} isAdmin={effectiveAdmin} storageKey={STORAGE_KEY} saveData={saveData} accent={ACCENT} lastSortObj={lastSortObj} />}
