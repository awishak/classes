// The game system, on the shared engine.
//
// COMM 118 and COMM 4 each had a copy of this file: 4203 lines and 4125 lines,
// and the difference between the two copies was a storage key, an accent, two
// category labels, a handful of multi-word surnames and one word in a URL.
// Everything else — the weekly game, Ten on Ten, Fishbowl, Team Trivia with its
// teams and its presenter screen, the rebound, the accolades — was the same
// code written out twice. COMM 2 had a third, shorter copy. So a fix went in
// three times or went in once and stayed broken in the other two.
//
// One copy now, rendered from a class config like the rest of the engine. The
// forked files stay where they are, frozen, because they still hold a term of
// games and grades at their own keys.
//
// Games live in the class store itself rather than at a key of their own. The
// game needs the roster and it awards points, and both the roster and the
// points log are already in that store: Around the Horn writes to the same
// `log`, and the gradebook reads it. A separate key would mean a second copy of
// the roster and points the gradebook could not see.
//
// The class the module is running for is held here rather than threaded
// through thirty components. A page shows one class: every route is
// `/<class>/...`, and the presenter opens in a window of its own. The entry
// points below set the class as they mount, and the helpers read the class
// from here.
...
// Where a class keeps its games, which is where a class keeps everything.
export const gameKeyOf = (config) => config?.storageKey || "";
const gkey = () => gameKeyOf(CFG);
...
// Only games played before 1 September carry a category, and each fork named
// the two categories differently: On Topic and Sports World in COMM 118, On
// Reading and Extra in COMM 4. A class can say its own names; nothing writes a
// category any more.
const DEFAULT_CATS = [
  { id: "on_topic", label: "On Topic" },
  { id: "sports_world", label: "Sports World" },
];
const cats = () => CFG?.gameCategories || DEFAULT_CATS;
...
// its own hard-coded list of the students whose surname does not fall on the
// last space; a class lists those in `lastNameOverrides`, the way the Ask page
// already reads them.
      ...
      return (
        <TriviaLiveAdmin
          ...
          onEndGameAndPool={() => {
            ...
            const confirmMsg = "End game now? " + (lockedQs.length > 0 ? "Locked round will be revealed. " : "") + (poolAdditions.length > 0 ? poolAdditions.length + " unused question" + (poolAdditions.length !== 1 ? "s" : "") + " will be saved to the pool. " : "") + "You'll go to the Finalize screen.";
    ...
    <div style={{ padding: "20px 20px 40px", fontFamily: themedHeadingFont(theme, F) }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        <div style={{ ...crd, padding: 16, marginBottom: 16 }}>
          ...
          {showShelf && (
            <QuestionPicker storageKey={bankKey()} mode="free"
  ...
  return (
    <div style={{ padding: "20px 20px 40px", fontFamily: themedHeadingFont(theme, F) }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        ...
        {picking ? (
          <QuestionPicker storageKey={bankKey()} mode="choice"
  ...
  const submitAnswer = async (actType, w, qIdx, answerIdx) => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        const verifyRaw = await window.storage.get(gkey(), true);
        if (verifyRaw?.value) {
          try {
...
*/
// The presenter opens in a window of its own, off a URL that names the class,
// so this is the one entry point that finds its class rather than being handed
// the class.
export function TriviaPresenter({ gameId, classKey }) {
  setGameClass(ENGINE[classKey]);
  const STORAGE_KEY = classKey === "comm118" ? gkey() : "comm4-v1";
